...
public class ExampleProducer {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ISerialize<PhemeSource> serializer = new GenericSerialize<PhemeSource>(PhemeSource.class);
		
		try {
			StreamProducer<PhemeSource> sp = new StreamProducer<PhemeSource>(conf, serializer);
			
			for (int i=0; i<1000; i++) {
				PhemeSource ps = new PhemeSource();
				ps.setText("Tweet number " + i);
				ps.setId("1235467" + i);
				ps.setCreatedAt(new Date().toString());
				ps.setUserScreenName("User1");
				
				ps.setRawJson("{\"test\": 2}");
